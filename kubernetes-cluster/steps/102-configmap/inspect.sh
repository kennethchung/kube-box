...
svcs=($(kubectl get svc  -n kube-box -o json | jq  '.items[]' | jq -rc '.metadata.name+"."+.metadata.namespace'))
for svc in "${svcs[@]}"
do
   echo $svc
   resp=$(kubectl exec -it  busybox  --  wget -O - http://"${svc}/json/custom.json")
   grep kube-box <<< $resp
done
...
do
   printgreen "${port}"
   resp=$(curl http://localhost:"${port}/json/custom.json")
   grep nginx <<< $resp
done